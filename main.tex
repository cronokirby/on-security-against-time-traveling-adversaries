...
While this is enough to break many existing schemes, it is nonetheless possible to
...
time, rewinding time backwards, or skipping time forwards.
...
we also present a fine grained analysis of different models of time travel.
We consider several variations of time travel, and show how they
form a hierarchy, with some models strictly more powerful
than others.
In particular, the ability to travel backwards is more powerful
than no time travel at all, and being able to travel both forwards and
...
the notion of time travel might seem irrelevant to
...
timelines, and on the depth of each timeline, showing that these restrictions
...
are broken, even in our weakest model of time travel.
...
games, rather than just for a particular cryptographic scheme.
...
The exports are those of $A$, with $\text{out}(A \circ B) = \text{out}(A)$,
and the imports are those of $B$, with $\text{in}(A \circ B) = \text{in}(B)$.
...
Given a pair of games $G_0, G_1$, we say that they are:
...
The security of a cryptographic scheme is captured by a pair of games $G_b$.
...
we would rather work with a \emph{single} interface, capable of capturing
the behavior of all of these games.

The key observation here is that the state of a game is modified in only two places:
...
information they get from querying its exported functions.
...
This models a very limited time machine which can only
make small backwards movements in time.
...
The game lets the adversary learn one key of their choice, but not
the other.
If the adversary manages to guess both of the keys, then they learn the value of $b$.
...
For certain parameters, $\text{Fork}(P, d)$ is actually equivalent to not having access
to time travel.
...
the adversary to fork at a position in $P' / P$, which demonstrates
the separation.
...
&\texttt{Fork}()\cr
&\texttt{Query}(0)\cr
&k_0 \gets \texttt{Wait}()\ d' \text{ times}\cr
&\texttt{Load}(1)\cr
&\texttt{Query}(1)\cr
&k_1 \gets \texttt{Wait}()\ d' \text{ times}\cr
&\texttt{Load}(1)\cr
&b \gets \texttt{Guess}(k_0, k_1)\cr
&\texttt{return } b\cr
\end{aligned}
...
First, we prove a general result, namely that for \emph{stateless}
games, time travel provides no advantage.

Then, we show a few more concrete results, in particular that while $\text{IND-CCA}$
...
either the encryption of that message, or a random message
...
We call this ciphertext a \emph{challenge}.
...
allowing them to decrypt ciphertexts of their choice.
Crucially, the adversary is \emph{not} allowed to decrypt any
...
In this family, the adversary has access to an oracle which can
